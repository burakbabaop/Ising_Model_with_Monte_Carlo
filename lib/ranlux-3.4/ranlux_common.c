...
*   ranlux_int (*state)[4];          Current state vectors and carry bits.
*
...
* reserved for the carry bits. The data type ranlux_int is defined in ranlux.h
...
static const ranlux_int base=(ranlux_int)(0x1000000000000);
static const ranlux_int mask=(ranlux_int)(0xffffffffffff);


int rlx_check_machine(void)
{
#if ((defined AVX2)||(defined SSE2))
   if ((sizeof(int*)>sizeof(unsigned long))||(sizeof(ranlux_int)!=8))
      return 1;
#endif
...
void rlx_alloc_state(rlx_state_t *s)
{
   ...
   (*s).state=(ranlux_int(*)[4])(((unsigned long)(addr+32))&(~mask));
#else
...
void rlx_init(rlx_state_t *s,int seed,int flag)
{
   ...
   for (i=0;i<4;i++)
   {
      for (k=0;k<24;k++)
      {
         ...
         if (k&0x1)
            state[k/2][i]+=((ranlux_int)(ix)<<24);
         else
            state[k/2][i]=(ranlux_int)(ix);
      }
   }

   (*s).state[12][0]=(ranlux_int)(0);
   (*s).state[12][1]=(ranlux_int)(0);
   (*s).state[12][2]=(ranlux_int)(0);
   (*s).state[12][3]=(ranlux_int)(0);
   (*s).ir=0;
}
...
{
   int pr,ir,k;
   ranlux_int shift,(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
...
{
   int pr,ir,k;
   ranlux_int d[4],(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
...
{
   int k;
   ranlux_int lmask,shift,(*state)[4];
   float onebit;

   onebit=(float)(ldexp(1.0,-24));
   lmask=(ranlux_int)(0xffffff);
   shift=(ranlux_int)(24);
   state=(*s).state;
...
{
   int k;
   ranlux_int sexp,(*state)[4];

   sexp=(ranlux_int)(0x4030000000000000);
   state=(*s).state;
...
void rlx_set_state(int *is,rlx_state_t *s)
{
   ...
   for (k=0;k<12;k++)
   {
      (*state)[0]=(ranlux_int)(is[0]);
      (*state)[1]=(ranlux_int)(is[1]);
      (*state)[2]=(ranlux_int)(is[2]);
      (*state)[3]=(ranlux_int)(is[3]);

      (*state)[0]+=((ranlux_int)(is[4])<<24);
      (*state)[1]+=((ranlux_int)(is[5])<<24);
      (*state)[2]+=((ranlux_int)(is[6])<<24);
      (*state)[3]+=((ranlux_int)(is[7])<<24);
